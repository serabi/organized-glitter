...
const logger = createLogger('csvImportExample');
...
export const handleCsvStringImport = async (csvContent: string) => {
  try {
    ...
    logger.info(`Successfully imported ${projects.projects.length} projects`);
...
export const handleCsvFileImport = async (file: File) => {
  try {
    ...
    logger.info(`Successfully imported ${projects.projects.length} projects from ${file.name}`);
    return projects.projects;
  } catch (error) {
    logger.error('CSV file import failed:', error);
    throw error;
  }
};
