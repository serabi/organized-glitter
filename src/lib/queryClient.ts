...
function handleQueryError(error: unknown, query: any): boolean {
  ...
  if (!query) {
    logger.criticalError('Query object is undefined in handleQueryError - this indicates a React Query bug or timing issue');
    ...
    queries: {
      ...
      retry: (failureCount, error) => {
        ...
        logger.debug('React Query retry attempt', {
          ...
          errorStatus: (error && typeof error === 'object' && 'status' in error) ? (error as any).status : undefined
        });
