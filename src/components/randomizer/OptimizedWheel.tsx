...
const CanvasWheel = memo<{
    ...
    const drawWheel = useCallback((currentRotation: number = 0) => {
        ...
        projectSegments.forEach(segment => {
            ...
            const text = segment.project.title.length > maxChars
        ...
        if (import.meta.env.DEV && renderTime > PERFORMANCE_THRESHOLDS.ANIMATION_PERFORMANCE_THRESHOLD && frameCountRef.current % 30 === 0) {
    ...
    }, [dimensions, projectSegments, size, projects.length, onPerformanceMetric, isSpinning, createOffscreenCanvas]);
...
const MemoizedWheelLabel = memo<{
    ...
    return (
        <div
            className="wheel__label wheel__label--optimized"
            style={labelStyle}
        >
            {displayText}
        </div>
...
export const OptimizedWheel: React.FC<OptimizedWheelProps> = memo(({
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(() => {
            ...
            const hasPerformanceIssues = avgRenderTime > PERFORMANCE_THRESHOLDS.RENDER_TIME_THRESHOLD_MS;
    ...
    const wheelGradient = useMemo(() => {
        ...
        if (projects.length > 40) {
            ...
            const gradientStops = simplifiedColors.map((color, index) =>
