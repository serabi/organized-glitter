...
export const StatsProvider: React.FC<StatsProviderProps> = ({ children }) => {
  ...
  const getCountsForTabs = useCallback((): CountsForTabsType | BadgeLoadingState => {
    ...
    return counts;
  }, [dashboardStats.stats?.status_breakdown, dashboardStats.isLoading, dashboardStats.isError, dashboardStats.error, isNetworkSlow]);
   ...
   */
  const getBadgeContent = useCallback((count: number): ReactNode => {
    const countsResult = getCountsForTabs();
    
    // Show spinner for loading and error states
    if (countsResult === 'loading' || countsResult === 'error') {
