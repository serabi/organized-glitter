...
export function useCacheCleaning() {
  ...
  const cleanByPattern = useCallback((pattern: string) => {
  ...
  const cleanByQueryKey = useCallback(<TQueryKey extends readonly unknown[]>(
    queryKey: TQueryKey,
    exact = false
  ) => {
    const removedQueries = queryClient.removeQueries({
  ...
  const cleanStaleQueries = useCallback((staleTimeMs = 5 * 60 * 1000) => {
    const now = Date.now();
    const removedQueries = queryClient.removeQueries({
      predicate: (query) => {
