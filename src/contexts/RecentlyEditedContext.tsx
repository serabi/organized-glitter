...
export const RecentlyEditedProvider: React.FC<RecentlyEditedProviderProps> = ({ children }) => {
   ...
   * @returns True if the project is recently edited
   */
  const isRecentlyEdited = useCallback((projectId: string): boolean => {
   ...
   * @param id - Project ID to set as recently edited, or null to clear
   */
  const setRecentlyEditedProjectIdWithLogging = useCallback((id: string | null) => {
  ...
  return (
    <RecentlyEditedContext.Provider value={contextValue}>
      {children}
